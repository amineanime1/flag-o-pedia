...
function checkVotingComplete(roomId) {
  const room = rooms.get(roomId);
...
async function startGame(roomId, settings) {
  ...
  room.questions = await generateQuestions(settings.mode, settings.totalQuestions);
      ...
      switch (type) {
        ...
        case 'vote': {
          ...
          if (room && room.gameState === 'voting') {
            if (!room.votingState.votes) room.votingState.votes = {};
